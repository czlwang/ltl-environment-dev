...
def add_basic_ltl(alphabets, add_flexible_state):
    ltls = []
    for a in alphabets:
        ba = Automaton(a, alphabets, add_flexible_state=add_flexible_state)
    ...
    for key, val in OP2NARG.items():
        ...
        ba = Automaton(ltl, alphabets, add_flexible_state=add_flexible_state)
...
def ltl_sampler(alphabets, env_name='',
    ...
    ltls = []; considered = set()
    if add_basics:
        ltls = add_basic_ltl(alphabets, add_flexible_state)
            ...
            if check_should_add(ltl, ba, include_templates, skip_templates, env_name=env_name):
                if paired_gen:
                    if check_should_add(new_ltl, new_ba, include_templates, skip_templates, env_name=env_name):
                        ...
                        new_ba = Automaton(new_ltl, alphabets, add_flexible_state=add_flexible_state)
