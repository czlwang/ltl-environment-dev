...
algo=$7  # a2c, acktr
failedonly=$8  # only update the failed transitions

env_flag="--env_name Craft --num_steps 20"
env_flag_train="--env_name Craft --num_steps 20"
gen_train_formula="--gen_formula_only --num_train_ltls $nformula"
gen_test_formula="--gen_formula_only --num_test_ltls 100"
baseline_flag=""
trans_flag=""

home="/storage/ylkuo/ltl-parser/ltl/"

train_path="--formula_pickle data/formula_Craft_${nformula}_${runid}.pickle"
...
if [ $failedonly = "true" ]; then
    trans_flag="--update_failed_trans_only"
fi
...
# train the model
python main.py $env_flag_train $trans_flag --algo $algo \
    ...
    --num_env_steps 300 --log_interval 100 \
    --load_eval_train \
    --image_emb_size 64 \
    --rnn_size $rnnsize \
    --rnn_depth $rnndepth \
    ...
    --lr $lr \
    --load_model_name craft_best_0622.pt
