...
def new_eval(true_symbols, tree):
    for i in range(len(tree)):
        node_val = get_node_val(tree[i])
        if node_val == '|' or node_val == '&':
            left_eval = new_eval(true_symbols, tree[i-1])
            right_eval = new_eval(true_symbols, tree[i+1])
            if node_val == '&':
                out =  left_eval & right_eval
            else:
                out =  left_eval | right_eval
            break
        elif node_val == '!':
            out = not new_eval(true_symbols, tree[i+1])
        elif node_val is not None and node_val not in ['(', ')']:
            if node_val == '1':
                out = True
            else:
                out = node_val in true_symbols
        elif node_val is None:
            out = new_eval(true_symbols, tree[i])
    return out


def new_eval_formula(true_symbols, formula, alphabets):
    tokens = formula.replace('(', '( ').replace(')', ' )').replace('!', '! ').split()
    tree = parse_simplebool(tokens, alphabets)
    out = new_eval(true_symbols, tree)
    return out
...
class Automaton(object):
            ...
            for branch in branches:
                ...
                for i in range(len(trans)):
                    if next_states[i] not in new_branches and \
                        ...
                        failed_trans.add(trans[i])
            if not prefix:
                return False, np.inf, set(), failed_trans
            branches = new_branches
            failed_trans = set()
        # expand the states by bfs to find the shortest distance to an accepting state
        dist_to_accept = 0; n_while = 0
        failed_trans = set(branches)  # set failed transition if not leading to accept
        while True:
    ...
    def random_transition(self, src):
        ...
        #tree = parse_simplebool(tokens, self._alphabets)
        #symbols = gen_symbols(tree)
...
if __name__ == '__main__':
    ...
    prefix, dist_to_accept, new_last_states, failed_trans = ba.is_prefix([['a'], ['a']], [])
    print(prefix, failed_trans)  # expect to be True, set()
    ...
    print(ba.gen_sequence())
    prefix, dist_to_accept, new_last_states, failed_trans = ba.is_prefix([['a']], [])
    print(prefix, failed_trans)  # expect to be False, not an empty set
    ba.draw('tmp_images/ba.svg', show=False)
