...
def setup_summary_writer(args):
    ...
    dir_name = 'runs/env=%s_seed=%s_datetime=%s_nformula=%i_lr=%s_alpha=%s_entropy=%s_rnnsize=%d_rnndepth=%d_algo=%s' \
    ...
    if args.update_failed_trans_only:
        dir_name = dir_name + '_failedonly'
...
def train(args, formulas):
    ...
    if args.algo == 'a2c':
        ...
        agent = A2C_ACKTRTrainer(ltl_tree, args.alphabets, args, acktr=True)
    else:
    ...
    for e in range(args.num_epochs):
        ...
        for i, f in enumerate(formulas):
            ...
            failed_ratio = train_formula(args, agent, envs, n_epoch=e, n_formula=i)
            n_iters += 1
            if n_iters % args.log_interval == 0:
                ...
                args.writer.add_scalar('failed_ratio', failed_ratio, n_update)
                for j, test_formula in enumerate(test_formulas):
                    args.formula = example_formula
                    n_successes, final_steps, n_formula = test(args, test_formula,
                           model_name=args.save_model_name + '_' + str(n_update) + '.pt')
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            for info in infos:
                ...
                if 'failed_components' in info.keys():
                    failed_components = failed_components.union(info['failed_components'])
            failed_ratio.append(len(failed_components)/float(agent.actor_critic.base.ltl_tree.size))
        ...
        if args.update_failed_trans_only:
            agent.actor_critic.freeze(failed_components)
        ...
        if len(episode_rewards) > 1:
            ...
            print(
                ...
                .format(j, total_num_steps,
                        ...
                        action_loss))
    return np.mean(np.array(failed_ratio))
...
def test(args, formulas, model_name):
    if not args.device:
